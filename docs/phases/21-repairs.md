...
| Work item | Status |
| --- | --- |
| Add 1: Add one Repair hull action while the ship is docked | DONE |
| Add 2: Require one timber cargo lot for the action | DONE |
| Add 3: Remove the timber when the player confirms the repair | DONE |
| Add 4: Restore a fixed amount of hull condition | DONE |
| Add 5: Show the repair amount before confirmation | DONE |
| Add 6: Disable the action when the hull is full or the ship has no timber | DONE |
| Complete when: Trade one timber lot for a clear amount of hull condition | DONE |
| Test 1: Damage the ship on a reef | DONE |
| Test 2: Put one timber lot in the ship cargo | DONE |
| Test 3: Dock and inspect the repair action | DONE |
| Test 4: Confirm the repair | DONE |
| Test 5: Confirm timber is gone and hull condition is higher | DONE |
