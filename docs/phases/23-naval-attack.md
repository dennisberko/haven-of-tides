...
| Work item | Status |
| --- | --- |
| Add 1: Add one broadside attack on each side of the player ship | DONE |
| Add 2: Show the active firing area before the shot | DONE |
| Add 3: Give the attack a short reload time | DONE |
| Add 4: Damage a target hull when it is in the firing area | DONE |
| Add 5: Show target hull condition and clear hit feedback | DONE |
| Add 6: Disable the target when its hull condition reaches zero | DONE |
| Complete when: Position and disable one target with broadsides | DONE |
| Test 1: Inspect one target | DONE |
| Test 2: Move it into a broadside firing area | DONE |
| Test 3: Fire and confirm hit feedback and lower hull | DONE |
| Test 4: Fire during reload and confirm that no shot occurs | DONE |
| Test 5: Continue until the target is disabled | DONE |
